...
const [userReactions, setUserReactions] = useState<{ [url: string]: 'up' | 'down' }>({});
const [reactionCounts, setReactionCounts] = useState<{ [url: string]: { up: number; down: number } }>({});
...
const fetchNews = useCallback(async () => {
  ...
  try {
    const res = await fetch(
      `/api/news/search?q=${encodeURIComponent(query)}&page=${currentPage}&pageSize=${pageSize}`
    );
    const data: NewsAPIResponse = await res.json();

    if (data.status === 'error') {
      setError(data.message || 'Terjadi kesalahan saat mencari berita');
    ...
    // Fetch public reaction counts
    if (fetchedArticles.length > 0) {
      const urls = fetchedArticles.map(a => a.url);
      const countsRes = await fetch(`/api/news/reactions?urls=${encodeURIComponent(JSON.stringify(urls))}`);
...
const handleRefresh = () => {
  ...
  if (articles.length > 0) {
    ...
    fetch(`/api/news/reactions?urls=${encodeURIComponent(JSON.stringify(urls))}`)
      .then(res => res.json())
      ...
      <p className="text-gray-600 dark:text-gray-400">
        {query ? (
          <>
            ...
            {totalResults > 0 && ` - ${totalResults} berita ditemukan`}
          </>
        ) : (
